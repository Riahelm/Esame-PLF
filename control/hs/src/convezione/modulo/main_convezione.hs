...
main_convezione nx dt | nx == 0 || nx == 1    = condizioneIniziale nx calcOndaQuadra lmtInf lmtSup
                      | otherwise             = calcConvTempo (condizioneIniziale nx calcOndaQuadra lmtInf lmtSup) nt nx c dx dt
                            where
