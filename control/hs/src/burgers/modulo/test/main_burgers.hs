...
main_burgers nx | nx == 0 || nx == 1 = condizioneIniziale nx calcOndaDenteSega lmtInf lmtSup
                | otherwise          = calcConvTempo (condizioneIniziale nx calcOndaDenteSega lmtInf lmtSup) nt nx nu dx dt
                     where
