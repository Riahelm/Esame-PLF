module MatchBurgers(main_burgers, calcConvTempo, calcConvSpazio) where
...
calcConvTempo :: [Double] -> Int -> Dati Double -> ([Double] -> Dati Double -> Double) -> 
                                                   ...
                                                   ([Double] -> Int -> Dati Double -> Double) -> [Double]
...
calcConvSpazio lx i dati sup pe | i == length lx - 1 = [sup lx dati]
                                | otherwise          = (pe lx i dati) : calcConvSpazio lx (i+1) dati sup pe 
...
passoEulero lx i (dx,dt) = (lx !! i - lx !! i * dt/dx * (lx !! i - lx !! (i-1)) + 
